...
// Add this utility function to extract the parent folder name from the data-for attribute
function getPreviousSegment(path: string): string | undefined {
...
export function ExplorerNode({ node, opts, fullPath, fileData }: ExplorerNodeProps) {
  ...
  // Get the parent folder name from the full path
  const parentFolderName = getPreviousSegment(folderPath)

  return (
    <>
      {node.file ? (
        // Single file node
        // Add logic to check if the file name matches the root folder name
          ...
          {node.name !== "" && (
            ...
            <div class="folder-container">
              ...
              <div key={node.name} data-folderpath={folderPath}>
                {folderBehavior === "link" ? (
                  <a href={href} data-for={node.name} className="folder-title">
                    {node.displayName}
                  </a>
                ) : (
                  <a href={href + node.name} data-for={node.name} className="folder-title" style={{ fontSize: "18px" }}>
                    {node.displayName}
                  </a>
                )}
              </div>
            ...
            >
              {node.children.map((childNode, i) => (
                <ExplorerNode
                  ...
                  fullPath={folderPath}
                  fileData={fileData}
                  rootFolderName={parentFolderName} // Pass the rootFolderName prop
                />
              ))}
            </ul>
          </div>
